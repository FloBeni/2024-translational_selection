...
pA =  ggplot(dt_graph,aes_string(y=ylabel,x=xlabel))  +
     geom_point(data=average_data , aes(x=x,
                                                        y= y,
                                                        group="moyenne"
  ),pch=21,fill=set_color[7],size=8)+
...
pB =  ggplot(dt_graph,aes_string(y=ylabel,x=xlabel))  +
  ...
  ylab("Average GC of abundant tRNA A/G") + 
  xlab("Average per gene GCi")
...
{
  pdf(file= paste(path_figure,"Figure7_bis.pdf",sep=""), width=10, height=4)
