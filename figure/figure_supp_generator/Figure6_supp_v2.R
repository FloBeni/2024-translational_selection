...
dt_graph$cut = str_replace_all(dt_graph$cut,",","-")
dt_graph$cut  = paste(dt_graph$cut ,", N = ", tapply(dt_graph$nb_genes,dt_graph$cut,max)[dt_graph$cut ]," genes",sep="")
dt_graph$cut = factor(dt_graph$cut,levels = rev(unique(dt_graph$cut)))

pA = ggplot(dt_graph[dt_graph$nb_genes >= max(dt_graph$nb_genes)*0.5,],aes(x = group , y= gci_count/posi_sites,fill=cut,col=cut)) +
  ...
  xlab("Distance from start codon or stop codon (kb, log scale)") + ylab ("GCi") + ylim(c(0.3,0.7))
pA

jpeg(paste(path_pannel,"p4A_supp.jpg",sep=""), width = 5500/1, height = 2200/1,res=500/1)
print(pA)
dev.off()
...
pB = ggplot(dt_graph[dt_graph$nb_genes >= max(dt_graph$nb_genes)*0.5,],aes(x = group,y= gci_count/posi_sites,fill=cut,col=cut)) +
...
jpeg(paste(path_pannel,"p4B_supp.jpg",sep=""), width = 5500/1, height = 2200/1,res=500/1)
print(pB)
dev.off()
...
{
  pdf(file= paste(path_figure,"Figure6_supp.pdf",sep=""), width=4, height=3.5)
