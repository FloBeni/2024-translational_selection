...
pA =  ggplot(dt_graph,aes_string(y=ylabel,x=xlabel))  +
  ...
  ) + scale_fill_manual("Clades",values=Clade_color) + xlab("Difference in POCs proportion between the top 5% and bottom 50% expressed (%)") +
  ylab("Difference in POCs proportion between the top 5% and\nbottom 50% expressed (%, accounting for POC-control)")
...
{
  pdf(file= paste(path_figure,"Figure3_supp.pdf",sep=""), width=6, height=4)
