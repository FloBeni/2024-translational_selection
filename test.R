...
GC <- read.table("GCi_GC3.tab")
tRNA <- read.table("tRNA_abundance.tab")
newick <- readLines('GTDrift_metazoa_phylogenetic_tree.nwk')
taxo <- read.table("phylogenie.tab", sep="\t")

rownames(code) = code$anticodon
code[tRNA_long$tRNA,]$aa_name
