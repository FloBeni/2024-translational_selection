...
# intronCoord_path = "/beegfs/data/fbenitiere/Projet-SplicedVariants/Annotations/Drosophila_melanogaster/formatted_data/IntronCoords.tab"
gff_path = sys.argv[3]
# gff_path = "/beegfs/data/fbenitiere/Projet-SplicedVariants/Annotations/Drosophila_melanogaster/data_source/annotation.gff"
...
for intron in intron_library.itertuples():
    if intron.strand == 1:
        intron_seq = str(genome[intron.seqname].seq[(intron.splice5+1):(intron.splice3-2)].upper())
    else:
    ...
    triplet_freq = pd.concat([triplet_freq,data_prot],ignore_index=True)
