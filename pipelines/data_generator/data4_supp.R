...
for (method_to_calculate in c("per_gene")){
  ...
  data4_supp = rbind(data4_supp,dt)
  ...
  dt$method_to_calculate = method_to_calculate
  data4_supp = rbind(data4_supp,dt[,colnames(data4_supp)])
}

write.table(data4_supp,"data/data4_supp.tab",quote=F,row.names = F,sep="\t")
